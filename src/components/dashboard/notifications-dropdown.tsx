...
export function NotificationsDropdown() {
  ...
  function handleNotificationTap(n: NotificationItem) {
    if (n.payload?.appointmentId) {
      setViewingAppointmentId(n.payload.appointmentId);
      setOpen(false); // close the dropdown so the dialog isn't hidden behind it
    }
  }
  ...
  return (
    <>
              ...
              {items.map((n) => (
                <div
                  ...
                  className={cn(
                    ...
                    !n.read && "bg-ivory",
                    n.payload?.appointmentId && "cursor-pointer hover:bg-ivory/70"
                  )}
                >
      ...
      <AppointmentDetailsDialog
        ...
        onOpenChange={(next) => !next && setViewingAppointmentId(null)}
      />
    </>
  );
}
